...
struct Args {
    ...
    /// Directory containing node_fills_streaming/, node_order_statuses_streaming/,
    /// and node_raw_book_diffs_streaming/, such as .../volumes/hl/data.
    #[arg(long)]
    ...
    /// Which WebSocket features to serve: comma-separated bbo, allbbo, l2book, l4book, trades, bookdiffs, orderupdates, stats, ordersync, or all
    #[arg(long, default_value = "all")]
    features: FeatureSet,
    ...
    /// Path to abci_state.rmp. Direct mode only.
    /// Default: <data_dir>/../hyperliquid_data/abci_state.rmp.
    /// This directory is a sibling of --data-dir and also contains visor_abci_state.json.
    #[arg(long)]
    ...
    /// Path to visor_abci_state.json for height information.
    /// Default: <data_dir>/../hyperliquid_data/visor_abci_state.json.
    #[arg(long)]
...
mod tests {
    ...
    fn cli_features_default_to_all() {
        ...
        assert!(args.features.ordersync());
    }
    ...
    fn cli_parses_each_single_feature() {
        for feature in
            ["bbo", "allbbo", "l2book", "l4book", "trades", "bookdiffs", "orderupdates", "stats", "ordersync"]
        {
            let args =
    ...
    fn cli_rejects_invalid_features() {
        ...
        assert!(Args::try_parse_from(["orderbook_server", "--features", "stats,stats"]).is_err());
        assert!(Args::try_parse_from(["orderbook_server", "--features", "ordersync,ordersync"]).is_err());
